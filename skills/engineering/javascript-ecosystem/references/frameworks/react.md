...
**Verified 2026-08-20.** Check the installed `react` version first; re-verify if newer than below.
...
- The Compiler is opt-in per build tool (Babel, Vite, Metro, Rsbuild) and assumes the Rules of React. Lint with `eslint-plugin-react-hooks` v7 (stable since Oct 2025, currently 7.1) at error before enabling.
