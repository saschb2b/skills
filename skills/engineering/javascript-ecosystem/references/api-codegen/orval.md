...
- The `zod` output targets Zod v4 by default since 8.19; a Zod v3 codebase must set the version explicitly in the zod config.
- Since 8.22, resolving external `$ref` documents requires opting in via `externalRefs` (a security fix); specs that split across files silently fail to resolve without it.
- Later 8.x adds `tags-operations` and `tags-operations-split` output modes and a global `throwOnError` option, so re-check the `output` reference before hand-rolling either.
