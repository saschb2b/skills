...
**From a live production site** (cloning a brand's shipped design system from its public pages), the stylesheet alone will lie to you: a long-lived site ships years of dead CSS beside the live rules. The procedure that survives contact, learned the hard way:
...
6. **Sweep again.** After the first pass, re-walk each fetched page and diff its module inventory against the bundle's coverage — the second pass is where the quicklink rows, stat bands, share rails, and back-to-top squares surface. Repeat the sweep for every new source page you are shown.
