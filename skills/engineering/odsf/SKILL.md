...
| `asset` | Author a companion `*.example.html` (or `*.do.html` / `*.dont.html`) plus its `styles/components.css` rules (consuming tokens via `var(--…)`), that renders standalone and shows the minimal correct markup. Foundations get demo sheets (swatches, specimens, scales); iconography gets a searchable icon browser with per-icon usage details. |
