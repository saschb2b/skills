...
| `content-flush-to-viewport-edge` | Page content (text, cards) sits flush against the viewport's left or right edge on a small screen because the outermost container has no horizontal gutter. | Give the outermost container a horizontal gutter (`padding-inline` on the page wrapper, or `margin-inline: auto` with `max-inline-size` and side padding) so content never touches the edge. | Full-bleed media, edge-anchored bars, deliberately edge-to-edge sections. | Untidy | Safe |
...
| `justified-text-no-hyphenation` | `text-align: justify` on prose without `hyphens: auto`, so word gaps stretch unevenly and rivers open down the column, worst in a narrow measure. | Left-align (`text-align: start`) prose; if justification is required, pair it with `hyphens: auto` and a wide enough measure. | Justified text with hyphenation in a wide measure that reads cleanly. | Untidy | Safe (to left-align), Judgment (keeping justification) |
...
Sources: [Anthony Hobday, Safe rules](https://anthonyhobday.com/sideprojects/saferules/), [CSS-Tricks, Aligning icons to text](https://css-tricks.com/tips-aligning-icons-text/), [NN/g, Zigzag layout](https://www.nngroup.com/articles/zigzag-page-layout/), [IBM Carbon, 2x grid](https://carbondesignsystem.com/elements/2x-grid/overview/), [MDN, Aligning items in a flex container](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_flexible_box_layout/Aligning_items_in_a_flex_container), [MDN, text-align](https://developer.mozilla.org/en-US/docs/Web/CSS/text-align), [MDN, hyphens](https://developer.mozilla.org/en-US/docs/Web/CSS/hyphens).
...
| `all-caps-running-text` | `text-transform: uppercase` (or hardcoded caps) on running body text or a long passage, which slows reading and usually lacks the slight tracking uppercase needs. | Reserve uppercase for short labels, eyebrows, and headings; set body in sentence case. Keep a short uppercase label only with `letter-spacing` around 0.05em to 0.1em. | Short labels, acronyms, single-word tags, a branded display heading. | Untidy | Judgment (casing is a style and content call) |
...
| `positioned-layer-clipped-by-overflow` | A tooltip, dropdown, popover, menu, or other absolutely-positioned layer is cut off at the edge of an ancestor whose `overflow` is hidden, auto, clip, or scroll, instead of escaping its box. | Render the layer in a portal at the document root, drop the clipping `overflow` on the axis that traps it, or anchor it with CSS anchor positioning so it is not a descendant of the clip. | A layer meant to scroll inside the container (an inner scroll region). | Glaring | Safe (drop the offending overflow), Judgment (portal or re-parent) |
...
Surface first (Judgment): `proximity-ignores-grouping`, `non-standard-grid`, `inconsistent-interaction-states` (when adding a state), `border-overuse`, `inconsistent-icon-style`, `inconsistent-decimal-precision`, `body-text-too-small`, `emphasis-by-size-only`, `too-many-typefaces`, `all-caps-running-text`, breakpoint strategy, and all of color and contrast.
