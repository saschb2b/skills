---
type: Library Notes
title: "i18next (with react-i18next)"
description: "i18next is now TypeScript-first."
tags: [javascript, i18n]
generated: { by: claude-code/unversioned, at: 2026-08-20T00:00:00Z }
---
# i18next (with react-i18next)

**Verified 2026-08-20.** Check the installed `i18next` and `react-i18next` versions first; re-verify if newer than below.

**Current stable**: i18next 26.4, react-i18next 17.0. **LLM default bias**: i18next v19 to v21 and react-i18next v11 to v13. Untyped string keys, the `withTranslation` HOC, custom `interpolation.format` callbacks, and TypeScript as an afterthought.
...
| `t('deeply.nested.key')` as the only option on huge catalogs | The Selector API `t($ => $.deeply.nested.key)`, opted in via `enableSelector` |
...
## Gotchas
- The Selector API is opt-in and changes call syntax; use the codemod for large codebases. `enableSelector` takes `true`, `'optimize'` (large catalogs), or `'strict'` (since 26.1, every path must start with an explicit namespace segment).
