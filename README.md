...
| Skill | What it does |
| --- | --- |
| **[no-slop](./skills/productivity/no-slop/SKILL.md)** | Write and revise human-facing prose in a plain, professional register, stripping the tells that mark text as AI-generated. Builds sentences with ASD-STE100 Simplified Technical English, strict for procedures and error messages, relaxed for prose. Two tables, twelve construction rules to write by and twenty tells to check against, plus before/after rewrites and a guard against over-correcting into robotic prose. Adds the Microsoft Writing Style Guide mechanics, plus a house-style mode for when a destination's style beats the register, and rules for prose that breaks with a screen reader or excludes a reader. Ships `slop-lint.mjs`, a zero-dependency linter scoring violations per 100 words. Catches list-itis and forced enumeration, not just em dashes and slop words. |
