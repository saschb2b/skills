...
| **opportunity-cost-loadout** | Equipping power for X means not equipping power for Y; the forecast is forced, with no "take all". | Additive loadouts drown the player and flatten every build into the same kitchen sink. | Sparklite, Moonlighter 2, Hollow Knight, Nier Automata (even the HUD is in the budget), Mina the Hollower (one weapon, one shield, one sidearm, a few of 60 trinkets) | One option strictly dominates; a reverse-engineerable optimum kills the choice. |
...
A move earns a catalog entry only when a second game exemplifies it (see [method.md](method.md)); until then it is a candidate, named but not curated, and tracking it is how the promotion pipeline stays honest. The current single-game candidates, all from Mina the Hollower:
